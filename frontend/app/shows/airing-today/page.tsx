...
const showsAiringToday = () => {

    const [showsAiringToday, setShowsAiringToday] = useState<TmdbTvResult[]>([]);
    const [pageCount, setPageCount] = useState<number>(1);

    useEffect(() => {
        axios.get('http://localhost:3001/shows/airing-today', {
            params: {
                page: pageCount
            }
        }).then((res) => {
            ...
            setShowsAiringToday((prevMovies) => [...prevMovies, ...showsWithMediaType])
    ...
    return (
        <MainLayout>
            <div className="grid gap-5 grid-cols-4 justify-items-center">
                {showsAiringToday.map((show, index) => (
                    <div
